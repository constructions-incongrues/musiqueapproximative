= API Subsonic
:description: Brancher un client Subsonic sur l'archive de Musique Approximative
...
. Sur la base de production, d'abord :
+
[source,sql]
----
ALTER TABLE post
...
. `make deploy RSYNC_PARAMETERS=` — la cible construit désormais `src/vendor`
  avant de synchroniser, donc getid3 part avec.
. `php symfony cache:clear` sur l'hôte.
. `php symfony musiqueapproximative:scan-tracks --limit=200` sur l'hôte pour
