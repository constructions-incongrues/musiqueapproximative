...
. *Création de l'élément* : Une div `.fish` est créée et ajoutée au `<body>`
. *Intégration Lottie* : Un iframe Lottie est intégré depuis `lottie.host`
. *Positionnement* :
 ** Position absolute
 ** Taille : 100x100px
. *Suivi du curseur* : Listener `mousemove` met à jour la position du poisson
. *Décalage vertical* : Le poisson est placé 20px au-dessus du curseur (`y - 20`)
