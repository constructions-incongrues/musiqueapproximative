...
. *Chargement* : Le script vérifie la présence des options dans `window.DesastreOptions.mamie`
. *Création de l'audio* : Crée un élément Audio avec l'URL du jingle
. *Configuration* :
 ** Volume configuré ou par défaut
 ** Vitesse de lecture aléatoire entre 0.5x et 1.5x
. *Lecture automatique* : Tente de lire automatiquement l'audio
. *Fallback* : Si l'autoplay est bloqué, attend une interaction utilisateur
