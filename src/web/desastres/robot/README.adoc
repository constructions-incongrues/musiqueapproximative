...
. *Sélection du titre* : Le script cible l'élément `article h2`
. *Transformation du texte* : Chaque occurrence de "robot" (case insensitive) est décomposée en lettres individuelles
. *Wrapping* : Chaque lettre est enveloppée dans deux `<span>` :
+
[,html]
----
...
. *Effet de perspective* : Utilisation de `perspective(40cm)` pour l'effet 3D
