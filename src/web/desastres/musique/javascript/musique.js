...
console.log('[desastres/musique] Loaded');
...
document.addEventListener("DOMContentLoaded", () => {
     console.log('[desastres/musique] DOMContentLoaded');
     const contentDiv = document.querySelector(".content");

     if (contentDiv) {
          console.log('[desastres/musique] Creating musical staff lines');
          for (let r = 0; r < 3; r++) {
     ...
     const walker = document.createTreeWalker(
          ...
          {
               acceptNode(node) {
                    if (!node.nodeValue || !/[Mm]/.test(node.nodeValue)) {
     ...
     while ((currentNode = walker.nextNode())) {
          ...
          for (const caractere of originalText) {
               if (caractere === "M" || caractere === "m") {
                    lettresM.push(caractere);
               } else {
                    nouveauTexte += caractere;
               }
          }

          if (nouveauTexte !== originalText) {
               currentNode.nodeValue = nouveauTexte;
          }
     }

     if (lettresM.length) {
          const conserve = Math.max(1, Math.ceil(lettresM.length / 3));
          lettresM.splice(conserve);
          const conserveSup = Math.max(1, Math.ceil(lettresM.length / 2));
          lettresM.splice(conserveSup);
          const conserveSup2 = Math.max(1, Math.ceil(lettresM.length / 2));
          lettresM.splice(conserveSup2);
     }

     window.musiqueLettresM = lettresM;
     ...
     const pathData = "M0,2c46.43,0,46.43,94,92.86,94S139.28,2,185.71,2s46.43,94,92.86,94S324.99,2,371.42,2s46.43,94,92.86,94S510.7,2,557.13,2s46.43,94,92.85,94S696.41,2,742.84,2s46.43,94,92.86,94S882.13,2,928.56,2s46.43,94,92.86,94S1067.84,2,1114.27,2s46.43,94,92.86,94S1253.57,2,1300,2";
     const stageId = "musique-lettres-stage";
     ...
     svgEl.setAttribute("viewBox", "0 0 1300 98");
     ...
     const lettersLayer = document.createElement("div");
     lettersLayer.className = "musique-lettres-layer";
     lettersLayer.style.position = "absolute";
     lettersLayer.style.top = "50%";
     ...
     stage.appendChild(lettersLayer);

     const letterElements = lettresM.map((caractere, index) => {
          const span = document.createElement("span");
          span.textContent = caractere;
          span.className = "musique-lettre";
     ...
     console.log('[desastres/musique] Animating', letterElements.length, 'letters along path');

     window.musiqueAnimeInstances = letterElements.map(({ element, index }) => {
          return anime({
               targets: element,
               translateX: path("x"),
               translateY: path("y"),
     ...
     console.log('[desastres/musique] Animations started');
});