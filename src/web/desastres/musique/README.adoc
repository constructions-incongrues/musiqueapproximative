...
. *TreeWalker* : Parcourt tous les nœuds texte du DOM
. *Filtrage* : Exclut les éléments `<script>` et `<style>`
. *Extraction* : Retire tous les "M" et "m" du texte
. *Réduction progressive* : Conserve environ 1/8 des lettres trouvées (3 réductions successives)
. *Stockage* : Les lettres retenues sont stockées dans `window.musiqueLettresM`

=== Phase 2 : Création de la scène

. *Conteneur* : Création d'un stage `#musique-lettres-stage` (100px de hauteur)
. *SVG Path* : Portée ondulée définie par un path complexe
. *Layer* : Couche dédiée pour les lettres animées

=== Phase 3 : Animation

. *anime.path()* : Utilise le path SVG comme trajectoire
. *Création des lettres* : Chaque lettre devient un `<span>` animé
. *Animation* :
