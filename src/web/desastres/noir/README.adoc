...
. *Création de l'overlay* : Une div avec la classe `.noir` est créée
. *Ajout au DOM* : La div est ajoutée comme enfant de `.content`
. *Effet visuel* : Les styles CSS (définis dans `style.css`) s'appliquent automatiquement
