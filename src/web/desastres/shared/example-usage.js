...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.DesastreAudio.onReady(function (audio) {
    ...
    setInterval(function () {
      if (audio.element && !audio.element.paused) {
        console.log(
          "[desastres/example] Current time:",
          audio.element.currentTime.toFixed(2) + "s",
        );
      }
    }, 1000);
  });
  ...
  window.DesastreAudio.onReady(function (audio) {
    ...
    console.log(
      "[desastres/example] Track duration is",
  ...
  setTimeout(function () {
    if (window.DesastreAudio && window.DesastreAudio.isReady) {
      console.log(
        "[desastres/example] Direct access works:",
        window.DesastreAudio.duration + "s",
      );
    } else {
      console.log(
        "[desastres/example] Audio not ready yet, use onReady() instead",
      );
    }
  }, 2000);
