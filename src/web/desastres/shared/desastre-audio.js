...
(function() {
    ...
    function triggerReadyCallbacks() {
        console.log('[desastres/shared/audio] Audio ready, duration:', window.DesastreAudio.duration + 's');
        ...
        window.DesastreAudio.readyCallbacks.forEach(function(callback) {
            try {
                ...
                console.error('[desastres/shared/audio] Error in ready callback:', error);
            }
        });
    ...
    function initAudio() {
        ...
        if (jPlayerAudio) {
            ...
            if (jPlayerAudio.duration && !isNaN(jPlayerAudio.duration) && jPlayerAudio.duration !== Infinity) {
    ...
    setTimeout(function() {
        if (!window.DesastreAudio.element) {
            console.log('[desastres/shared/audio] Retrying audio element detection...');
            initAudio();
        }
    }, 500);
