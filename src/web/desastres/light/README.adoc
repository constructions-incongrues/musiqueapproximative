...
. *Sélection du conteneur* : Le script cible l'élément `.content`
. *Création de 3 rayons* : Trois div avec la classe `.light` sont créées
. *Ajout au DOM* : Les divs sont ajoutées comme enfants de `.content`
. *Animation* : Chaque rayon est animé avec des paramètres légèrement différents
