...
if (!window.DesastreOptions || !window.DesastreOptions.postillons) {
  console.error(
    "[desastres/postillons] ERROR: Options not found in window.DesastreOptions.postillons",
  );
  console.log(
    "[desastres/postillons] Available options:",
    window.DesastreOptions,
  );
} else {
  console.log(
    "[desastres/postillons] Options loaded:",
    window.DesastreOptions.postillons,
  );

  // Options avec valeurs par défaut
  const options = {
    imageFolder:
      window.DesastreOptions.postillons.imageFolder ||
      "/desastres/postillons/img/default",
  ...
  function creerpostillons() {
    ...
    if (plateau) {
      ...
      positions.forEach((pos, index) => {
        setTimeout(() => {
          ...
          image.onerror = function () {
            console.error(
              `[desastres/postillons] ERROR: Image not found: ${imagePath}`,
            );
      ...
      console.log(
        `[desastres/postillons] ${options.imageCount} tombs will be created progressively (${options.delay}ms delay)`,
      );
    } else {
