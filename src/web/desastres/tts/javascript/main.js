...
if (!window.DesastreOptions || !window.DesastreOptions.tts) {
  console.error(
    "[desastres/tts] ERROR: Options not found in window.DesastreOptions.tts",
  );
  ...
  function buildContext() {
    ...
    if (window.DesastreAudio && window.DesastreAudio.isReady) {
      context.audio = {
        ...
        currentTime: window.DesastreAudio.element
          ? window.DesastreAudio.element.currentTime
          : null,
      };
    }
  ...
  async function getText() {
    ...
    if (window.DesastreOptions.tts.url) {
      console.log(
        "[desastres/tts] Fetching text from URL:",
        window.DesastreOptions.tts.url,
      );
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          console.log(
            "[desastres/tts] Text selected from URL (",
            data.length,
            "options):",
            randomText.substring(0, 100) +
              (randomText.length > 100 ? "..." : ""),
          );
          return randomText;
        } else {
          console.error(
            "[desastres/tts] ERROR: URL did not return a valid array of texts",
          );
        }
      } catch (error) {
        console.error(
          "[desastres/tts] ERROR: Failed to fetch text from URL:",
          error,
        );
      }
    }
    ...
    if (
      window.DesastreOptions.tts.texts &&
      Array.isArray(window.DesastreOptions.tts.texts) &&
      window.DesastreOptions.tts.texts.length > 0
    ) {
      const texts = window.DesastreOptions.tts.texts;
      const randomText = texts[Math.floor(Math.random() * texts.length)];
      console.log(
        "[desastres/tts] Text selected from configured list (",
        texts.length,
        "options):",
        randomText.substring(0, 100) + (randomText.length > 100 ? "..." : ""),
      );
      return randomText;
    }
    ...
    if (!element) {
      console.error(
        "[desastres/tts] ERROR: Element not found with selector:",
        selector,
      );
      return null;
    }
    ...
    console.log(
      "[desastres/tts] Text found from selector (length:",
      text.length + " chars):",
  ...
  function getValueOrRandom(configValue, defaultMin, defaultMax) {
    if (configValue === undefined) {
      ...
      console.warn(
        "[desastres/tts] WARNING: Invalid config value, using default random range",
      );
      return defaultMin + Math.random() * (defaultMax - defaultMin);
    }
  }
  ...
  async function speakText() {
    ...
    if (window.DesastreOptions.tts.lang !== undefined) {
      ...
      utterance.lang =
        frenchLangs[Math.floor(Math.random() * frenchLangs.length)];
    ...
    if (Array.isArray(window.DesastreOptions.tts.rate)) {
      console.log(
        "[desastres/tts] Using random rate from range",
        window.DesastreOptions.tts.rate,
        ":",
        utterance.rate.toFixed(2),
      );
    } else if (window.DesastreOptions.tts.rate !== undefined) {
      console.log("[desastres/tts] Using configured rate:", utterance.rate);
    } else {
      console.log(
        "[desastres/tts] Using default random rate:",
        utterance.rate.toFixed(2),
      );
    }
    ...
    if (Array.isArray(window.DesastreOptions.tts.pitch)) {
      console.log(
        "[desastres/tts] Using random pitch from range",
        window.DesastreOptions.tts.pitch,
        ":",
        utterance.pitch.toFixed(2),
      );
    } else if (window.DesastreOptions.tts.pitch !== undefined) {
      console.log("[desastres/tts] Using configured pitch:", utterance.pitch);
    } else {
      console.log(
        "[desastres/tts] Using default random pitch:",
        utterance.pitch.toFixed(2),
      );
    }

    // Volume: nombre fixe ou plage [min, max]
    utterance.volume = getValueOrRandom(
      window.DesastreOptions.tts.volume,
      0.3,
      1,
    );
    if (Array.isArray(window.DesastreOptions.tts.volume)) {
      console.log(
        "[desastres/tts] Using random volume from range",
        window.DesastreOptions.tts.volume,
        ":",
        utterance.volume.toFixed(2),
      );
    } else if (window.DesastreOptions.tts.volume !== undefined) {
      console.log("[desastres/tts] Using configured volume:", utterance.volume);
    } else {
      console.log(
        "[desastres/tts] Using default random volume:",
        utterance.volume.toFixed(2),
      );
    }
    ...
    const setVoice = () => {
      ...
      console.log(
        "[desastres/tts] Available voices count:",
      ...
      if (
        window.DesastreOptions.tts.voices &&
        Array.isArray(window.DesastreOptions.tts.voices) &&
        window.DesastreOptions.tts.voices.length > 0
      ) {
        // Cas 1: options.voices est defini -> choisir parmi cette liste
        const voiceNames = window.DesastreOptions.tts.voices;
        const randomVoiceName =
          voiceNames[Math.floor(Math.random() * voiceNames.length)];
        console.log(
          "[desastres/tts] Selecting random voice from configured list (",
          voiceNames.length,
          "options):",
          randomVoiceName,
        );
        ...
        selectedVoice = availableVoices.find(
          (voice) => voice.name === randomVoiceName,
        );

        if (selectedVoice) {
          console.log(
            "[desastres/tts] Voice set:",
            selectedVoice.name,
            "(lang:",
            selectedVoice.lang + ")",
          );
          ...
          console.warn(
            "[desastres/tts] WARNING: Configured voice not found:",
            randomVoiceName,
          );
          console.log(
            "[desastres/tts] Available voices:",
            availableVoices.map((v) => v.name).join(", "),
          );
        }
      } else {
        // Cas 2: options.voices n'est pas defini -> choisir parmi toutes les voix disponibles
        console.log(
          "[desastres/tts] No voice list configured, selecting random voice from all available voices",
        );
        ...
        if (availableVoices.length > 0) {
          ...
          console.log(
            "[desastres/tts] Random voice selected:",
            selectedVoice.name,
            "(lang:",
            selectedVoice.lang + ")",
          );
          ...
          console.warn(
            "[desastres/tts] WARNING: No voices available, using default voice",
          );
        }
      }
  ...
  const speakOnInteraction = () => {
    console.log(
      "[desastres/tts] 👆 User interaction detected, triggering speech",
    );
    speakText();
  ...
  console.log(
    "[desastres/tts] Listeners registered for user interaction fallback",
  );
}
